...
def getRunNumber(fastq_path):
    ...
    :param str fastq_path: a path to a fastq_file that, critically, includes the containing directory run_#####_samples/
    :raise: AttributeError if no pattern matching run_\d* is found
    :returns: the run number extracted from the path as string (leading zero will be retained)
    :rtype: str
    """
    try:
        regex = r"run_(\d*)"
...
def decomposeStatus2Bit(status):
    """
        ...
        eg 18 = 2 + 16 or the powers of 2 [1.0,4.0]. See the qc config thresholds
